...
*/
// template <class keyValItem>
// int BTree<keyValItem>::search(keyValItem &target)
// {
...
/**
 * Decide to visit which branch in each node
 * if key in target found in *current, position will be the position of the target in this node, target will be updated.
 * if fail, return NOT_PRESENT and position will be the position of the child branch need for continue searching 
*/
// int searchNode(BNode *current, const keyValItem &target, int &position)
// {
//     position = 0;
//     while (position < current->count && target.key > current->keys[position].key)
//         position++;
//     if (position < current->count && target.key == current->keys[position].key)
//         return SUCCESS;
//     else
//         return NOT_PRESENT;
// }
...
bool removeBNode(BTree* tree, BNode* node, char* k) {
    ...
    if (idx < node->count && strcmp(node->keys[idx].key, k) == 0) {
        ...
        if (flag && idx > node->count) {
            ...
            removeBNode(tree, sibling, k);
            diskWrite(tree, sibling, sibling->pos);
        }
        else
            removeBNode(tree, c_i, k);
        
        diskWrite(tree, c_i, c_i->pos);
        res = true;
    }
    return res;
}
